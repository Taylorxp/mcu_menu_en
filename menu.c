...
static int32_t _PresetSpeed(struct menu_item *pthis, int32_t todo);
...
{
  #if defined(HD30S) || defined(HD30W3) || defined(HD35W3)
    {"DZOOM:              ", _Dzoom},
	#if defined(HD30S)
...
const struct menu_content menu_picture[] =
{
    ...
    //    {"HUE:                ", _Hue},
    {"GAMMA:              ", _Gamma},
#if defined(HD30M4) || defined(HD30W3) || defined(HD35W3)
#else
    {"WDR:                ", _Wdr},
#endif
#if defined(HD30M4) || defined(HD30W3) || defined(HD35W3)
...
static int32_t _PresetSpeed(struct menu_item *pthis, int32_t todo)
{
    ...
    {
        case MENU_REFRESH:
        memcpy(&pthis->text[20-3], NumToString(stSysInfo.preset_speed), 3);
        break;
...
static int32_t _PictureFlip(struct menu_item *pthis, int32_t todo)
{
    ...
    {
        case MENU_REFRESH:
        memcpy(&pthis->text[20-3], strOnOff[stSysInfo.flip^3], 3);
        break;
        
        case MENU_LEFT:
        case MENU_RIGHT:
        if(stSysInfo.flip == VISCA_ON) stSysInfo.flip = VISCA_OFF;
        else stSysInfo.flip = VISCA_ON;
        cam_flip(stSysInfo.flip);
        ShowCurLine();
        break;
    }
    return 0;
...
static int32_t _PictureMirror(struct menu_item *pthis, int32_t todo)
{
    ...
    {
        case MENU_REFRESH:
        memcpy(&pthis->text[20-3], strOnOff[stSysInfo.mirror^3], 3);
        break;
        
        case MENU_LEFT:
        case MENU_RIGHT:
        if(stSysInfo.mirror == VISCA_ON) stSysInfo.mirror = VISCA_OFF;
        else stSysInfo.mirror = VISCA_ON;
        cam_mirror(stSysInfo.mirror);
        ShowCurLine();
        break;
    }
    return 0;
...
* Description:  曝光
***************************************************************************************************/
#define SHUTTER_DELAY()       delay_ms(100)
static int32_t _Exposure(struct menu_item *pthis, int32_t todo)
{
    switch(todo)
    {
        ...
        case MENU_ENTER:
        {
            ...
            if(stCamInfo.ae_mode == VISCA_AE_MANUAL)
            {
              ...
              #if defined(HD32W3)
                cam_shutter_direct(stCamInfo.shutter);
                SHUTTER_DELAY();
              #endif
              
#if defined(EN_IRIS_BRIGHT)
            ...
            else if(stCamInfo.ae_mode == VISCA_AE_SHUTTER_PRIORITY)
            {
              ...
              #if defined(HD30S) || defined(HD30W) || defined(HD32W) || defined(HD32W3) || defined(HD35W)
              #else
...
#elif defined(HD30M4) || defined(HD30W3) || defined(HD35W3)
		if(stCamInfo.ae_mode == VISCA_AE_FULL_AUTO) stCamInfo.ae_mode = VISCA_AE_MANUAL;
		else stCamInfo.ae_mode = VISCA_AE_FULL_AUTO;
#else
        if(stCamInfo.ae_mode == VISCA_AE_FULL_AUTO) stCamInfo.ae_mode = VISCA_AE_MANUAL;
        else if(stCamInfo.ae_mode == VISCA_AE_MANUAL) stCamInfo.ae_mode = VISCA_AE_SHUTTER_PRIORITY;
        else stCamInfo.ae_mode = VISCA_AE_FULL_AUTO;
#endif
...
#else        
        if(stCamInfo.ae_mode == VISCA_AE_FULL_AUTO) stCamInfo.ae_mode = VISCA_AE_SHUTTER_PRIORITY;
        else if(stCamInfo.ae_mode == VISCA_AE_SHUTTER_PRIORITY) stCamInfo.ae_mode = VISCA_AE_MANUAL;
        else stCamInfo.ae_mode = VISCA_AE_FULL_AUTO;
#endif
        cam_ae_mode(stCamInfo.ae_mode);
        DeletMenu();
    ...
    switch(todo)
    {
        ...
        case MENU_ENTER:
        {
            ...
            if(stCamInfo.wb_mode == VISCA_WB_MANUAL)
            {
              ...
              #if defined(HD32W3)            
                cam_rgain_direct( stCamInfo.rgain );
                WB_DELAY();
                cam_rgain_direct( stCamInfo.rgain );
                WB_DELAY();
                cam_bgain_direct( stCamInfo.bgain );
                WB_DELAY();
                cam_bgain_direct( stCamInfo.bgain );
                WB_DELAY();
              #endif
...
static int32_t _Sharpness(struct menu_item *pthis, int32_t todo)
{
    ...
    {
        case MENU_REFRESH:
        memcpy(&pthis->text[20-3], NumToString(stCamInfo.aperture), 3);
        break;
        case MENU_LEFT:
        cam_aperture(VISCA_DOWN);
        ShowCurLine();
        break;
        case MENU_RIGHT:
        cam_aperture(VISCA_UP);
        ShowCurLine();
        break;
    }
    return 0;
...
static int32_t _IrAddress(struct menu_item *pthis, int32_t todo)
{
    switch(todo)
    {
        ...
        case MENU_LEFT:
        if(stSysInfo.ir_addr > 1) stSysInfo.ir_addr--;
